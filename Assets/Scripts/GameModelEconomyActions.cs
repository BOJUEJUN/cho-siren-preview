...
namespace ChoSiren
{
    ...
    public sealed partial class GameModel
    {
        ...
        public bool TryExchangeDiamondsForGold(out string message)
        {
            ...
            SaveState();
            message = $"兑换成功：金币 +{quote.ReceiveAmount:N0}，星钻 -{quote.DiamondCost:N0}";
            return true;
        }
        ...
        public string CurrencyHelpDescription(string currencyId)
        {
            switch (currencyId)
            {
                ...
                case CurrencyIds.Gold:
                    return $"金币 · 养成资源\n用途：训练成员、面试签约、强化饰品。\n获取：关卡通关、任务和舞台收益。\n也可用 {GoldExchangeDiamondCost} 星钻兑换 {GoldExchangeAmount:N0} 金币，确认后才会扣除。";
