...
namespace ChoSiren.Tests
{
    public sealed class GameModelTests
    {
        ...
        [Test]
        public void FixedStageEnemiesNeverScaleWithPlayerLevelAndKeepExistingSave()
        {
            string[] fixedEnemies = null;
            foreach (int level in new[] { 1, 68, 100 })
            {
                SaveRaw(new GameSave { StoryProgress = 100, EquippedAccessory = 0,
                    MemberLevels = Enumerable.Repeat(level, GameModel.Members.Length).ToList() });
                var model = new GameModel(() => now);
                int gold = model.Save.Gold, diamonds = model.Save.Diamonds, stamina = model.Save.Stamina;
                int[] levels = model.Save.MemberLevels.ToArray(), team = model.Save.Team.ToArray();
                BattleSimulator battle = model.StartStageBattle("stage-1-10", 17, out string message);
                Assert.That(battle, Is.Not.Null, message);
                Assert.That(message, Does.Not.Contain("适配"));
                Assert.That(battle.EnemyHpMultiplierPermille, Is.EqualTo(1000));
                string[] enemies = battle.Units.Where(unit => unit.Side == BattleSide.Enemy)
                    .Select(unit => $"{unit.Definition.Id}/{unit.MaxHp}/{unit.BaseAttack}/{unit.BaseDefense}").ToArray();
                if (fixedEnemies == null) fixedEnemies = enemies;
                CollectionAssert.AreEqual(fixedEnemies, enemies, "同一关卡不能随玩家等级暗中放大敌方属性。");
                Assert.That(model.EnemyPowerOfStage("stage-1-10"), Is.EqualTo(battle.Units
                    .Where(unit => unit.Side == BattleSide.Enemy)
                    .Sum(unit => unit.MaxHp / 10 + unit.BaseAttack * 5 + unit.BaseDefense)));
                var reloaded = new GameModel(() => now);
                CollectionAssert.AreEqual(levels, reloaded.Save.MemberLevels);
                CollectionAssert.AreEqual(team, reloaded.Save.Team);
                Assert.That(reloaded.Save.Gold, Is.EqualTo(gold));
                Assert.That(reloaded.Save.Diamonds, Is.EqualTo(diamonds));
                Assert.That(reloaded.Save.Stamina, Is.EqualTo(stamina - battle.Stage.StaminaCost));
                Assert.That(reloaded.Save.StoryProgress, Is.EqualTo(100));
            }
        }
