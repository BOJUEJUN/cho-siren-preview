...
用户已明确要求“开始演出”进入原冒险剧本页面、删除独立冒险剧本按钮，并要求“取消适配吧”。本轮实施与验收见 `Docs/LOBBY-ENTRY-FIXED-STAGES-2026-09-05.md`。适配试演不是企划要求，下面7de29ac/9e90542对应的该模式是历史记录，不得重新恢复。保留原角色成长与存档，固定关卡允许高等级轻松通关；保留单排操作栏、“手动重投/自动重投”和角色持续自动攻击，不在此批改特效或其他玩法。

20:14该批已上线Pages `5a8d99d`，游戏二进制源码 `ece32db`。272项逻辑、48项界面测试通过，新包旧档及官网新档实际入口/战斗/结算/返回/双面试池已验收；新档1-1为12秒三星胜利，体力按关卡扣除一次，进入地图不扣费。官网四个资源HTTP200、HTML与版本清单一致。当前用户页面可直接刷新，不要求改用本地网址。

## 最新资料与后续单次授权

17:18用户报告a7d14c7中1-10仅3.6秒取胜，接受角色自动行动；随后要求“你自己检查优化”。本次单批优化以 `7de29ac` 为二进制源码，在不削减旧档等级/资产、不改变原关卡奖励费用的条件下，加入地图明确标注且可切回原强度的“适配试演”，并将骰子按钮标为“手动重投／自动重投”、计时标为“剩余”。普通攻击和双技能仍自动。275项逻辑、48项运行时界面测试、5项发布暂存测试、12项网页加载器测试通过；隔离浏览器新档首关8.3秒、合成旧档1-10试演13.9秒取胜。Pages `9e90542`已推送，18:02官网四个资源HTTP200、HTML与清单与本机构建一致；正式站交互验收结果见 `Docs/HIGH-LEVEL-REHEARSAL-2026-09-05.md` 的最终追加。原强度仍允许高等级碾压；不要把适配模式描述为企划强制要求或悄悄强化了普通关。此项也不恢复无限目标，不宣称全部游戏优化完成。

18:06官网交互复查已完成：隔离合成旧档1-10适配试演约18秒三星胜利、0倒下；自动重投、真实结算和返回成员/双面试池均正常，无pageerror。只有这个正式站实测与上述测试/构建共同构成本批上线验收，不冒充真机或整款游戏验收。

以下为此前版本历史，不要用其构建覆盖9e90542：
...
当前开发分支是 `ui/battle-layout-20260905`，而不是下文历史快照中的 master。当前Pages为 `5a8d99d`（游戏二进制源码 `ece32db`，上一正式版 `9e90542`），已合并大厅入口并取消适配试演；保留此前门面职业、实时战斗、成长/章节节奏、伤害飘字分离、训练、战力/饰品数据同源、HUD、地图和双面试池。最新验收见 `Docs/LOBBY-ENTRY-FIXED-STAGES-2026-09-05.md`；适配模式文档仅为历史记录。继续遵循 `Docs/WEBGL-RELEASE-SAFETY-2026-09-05.md`，用 `Tools/Stage-WebGL.mjs` 保留上一版资源及版本清单，不再手动删成四个文件。不能仅从旧 origin/master 重建覆盖官网；源码推送暂受限制，不把已发布网页等同于源码已远端同步。
