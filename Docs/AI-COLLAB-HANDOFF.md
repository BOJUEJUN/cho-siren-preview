...
当前开发分支是 `ui/battle-layout-20260905`，而不是下文历史快照中的 master。最新已验证 Pages 为 `f511fd4`（源码 `6e0f1a2`）：真实战力/饰品数据同源及窄屏对比，保留此前训练、战斗、HUD、地图和选秀布局。详见最新企划对照文档。不能仅从旧 origin/master 重建覆盖官网；源码推送暂受限制，不把已发布网页等同于源码已远端同步。
