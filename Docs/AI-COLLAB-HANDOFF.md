...
当前开发分支是 `ui/battle-layout-20260905`，而不是下文历史快照中的 master。当前Pages为 `9e90542`（游戏二进制源码 `7de29ac`，上一正式版 `a7d14c7`），保留此前门面职业、实时战斗、成长/章节节奏、伤害飘字分离、训练、战力/饰品数据同源、HUD、地图和双面试池。新适配试演的详细验收见 `Docs/HIGH-LEVEL-REHEARSAL-2026-09-05.md`；此前版本记录见 `Docs/RELEASE-2026-09-05-DOCUMENT.md`。继续遵循 `Docs/WEBGL-RELEASE-SAFETY-2026-09-05.md`，用 `Tools/Stage-WebGL.mjs` 保留上一版资源及版本清单，不再手动删成四个文件。不能仅从旧 origin/master 重建覆盖官网；源码推送暂受限制，不把已发布网页等同于源码已远端同步。
