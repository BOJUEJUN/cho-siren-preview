...
16:00结束后，用户新发企划（2）并要求“尽量按照文档的来吧，虽然文档也可能有错”。当前依据见 `Docs/PPTX-2-ALIGNMENT-2026-09-05.md`。新版第11页第四职业改为**门面**；以下历史记录中的DJ不再是当前公开职业。已对齐源码、清单、筛选和编队，旧DJ只用于兼容读取；271项逻辑、46项运行时界面测试通过。本次为明确的文档对齐增量，不恢复无限期目标或绕过发布验收，未重建WebGL/未发布，网站并未自动更新。下午035a0ab候选不含门面，不能直接上传冒充本次更新。
...
当前开发分支是 `ui/battle-layout-20260905`，而不是下文历史快照中的 master。最新已验证 Pages 为 `6693a99`（游戏二进制 `6e0f1a2`，启动模板/发布工具 `f6a6a55`）：真实战力/饰品数据同源、窄屏对比与缓存版本保护，保留此前训练、战斗、HUD、地图和选秀布局。详见最新企划对照文档和 `Docs/WEBGL-RELEASE-SAFETY-2026-09-05.md`。以后用 `Tools/Stage-WebGL.mjs` 保留上一版资源及版本清单，不再手动删成四个文件。不能仅从旧 origin/master 重建覆盖官网；源码推送暂受限制，不把已发布网页等同于源码已远端同步。
