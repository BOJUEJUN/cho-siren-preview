...
16:00结束后，用户新发企划（2）并要求“尽量按照文档的来吧，虽然文档也可能有错”，随后明确要求“上线”。当前依据见 `Docs/PPTX-2-ALIGNMENT-2026-09-05.md`。新版第11页第四职业改为**门面**；以下历史记录中的DJ不再是当前公开职业。已对齐源码、清单、筛选和编队，旧DJ只用于兼容读取；271项逻辑、46项运行时界面测试通过。17:04发布验收完成：从源码 `2237c51` 重新构建，正式Pages已上线 `a7d14c7`；4个新资源HTTP 200、HTML/清单匹配，官网门面筛选和首关胜利已实测。详见 `Docs/RELEASE-2026-09-05-DOCUMENT.md`。这是用户明确授权的单次发布，不恢复无限期优化目标。下午035a0ab候选不含门面，不能再上传覆盖本次新包。

## 本轮停止边界（2026-09-05 14:39 用户追加）

用户要求“优化目标改成优化到4点”。当前时间为北京时间14:39，已明确向用户说明按**当天16:00**收尾；若用户后续改口，以其新指令为准。每次目标续跑、启动测试/构建/发布之前读取当前时间。到16:00停止新增优化、构建和发布，不为赶时间上线未验证内容；仅安全收尾已运行操作、保留修改和存档、汇报已上线/未上线/剩余问题。没有继续工作的默认授权，不能把长期目标标记为全部完成来冒充定时暂停。目标工具本身没有期限编辑或强制暂停接口。

15:07 实时战斗基础检查点：独立普攻/双技能计时、伤害能量、整场重投额度、暂停倍速、四族技能与冷却文字已接入。EditMode v3 251/251、PlayMode v4 44/44 通过，内部 WebGL 已跑到真实胜利结算。**尚未发布**：展示用高等级队伍在旧关卡只需1.65–1.95秒，不能算平衡合格。下一批须检查普通/精英/首领类型、成长曲线和正常新手存档，不以本轮绿灯测试冒充完整游戏完成。内部构建 `WebGL-Realtime-Review-20260905` 早于最终提示文案小修，不能直接作为最终产物上传。官网仍为6693a99，二进制6e0f1a2。

Unity：`6000.6.0f1 (f7f8ed4d1e24)`

15:34增量：已将第一章、真实成长/训练和种族技能档案接入同一套实时数据。266项逻辑、44项运行时测试通过；四队长640场队列验证见 `Docs/REALTIME-PROGRESSION-2026-09-05.md`。这不是完整企划完成，网页构建/视觉验收/上线状态必须看该文末追加记录。没有获准改变本轮16:00停止边界。

15:53网页验收：d370dc8内部构建的新档培养、旧68级档案迁移、战斗胜利和保留页面已目视检查，但伤害飘字出现同步重叠，禁止上传该构建。已修复并通过45/45 PlayMode；需要以修复后源码重新构建、实测，不得因临近16:00跳过门禁。详见上述验收记录。

15:58收尾：`035a0ab`候选已成功构建，内部浏览器确认伤害飘字分离及进度保留，发布工具仅dry-run通过。未正式暂存或推送，官网仍6693a99。候选目录为 `Builds/WebGL-Progression-Candidate-20260905`，具体散列、截图、测试和未完成项见验收记录。按16:00截止停止本轮，不以目标自动续跑恢复优化；需用户新授权继续。
设计基准：竖屏 `720 × 1536`
主场景：`Assets/Scenes/Main.unity`

新电脑接手先读：`Docs/GETTING_STARTED.md`、`Docs/CONTINUITY.md` 和本文的文件所有权、验收清单与当前事实。

最新企划与实施边界：`Docs/PPTX-2-ALIGNMENT-2026-09-05.md`（企划（2），33页）。`Docs/PPTX-ALIGNMENT-2026-09-05.md` 是企划（1）的历史实施记录，`Docs/LATEST-DESIGN-2026-09-05.md` 是更早PDF分析。当前四职业为主唱、主舞、Rapper、门面，骰型倍率上限2.0；旧DJ仅作兼容输入。

当前开发分支是 `ui/battle-layout-20260905`，而不是下文历史快照中的 master。最新已验证 Pages 为 `a7d14c7`（游戏二进制源码 `2237c51`，上一正式版 `6693a99`）：第四职业门面、实时战斗、成长/章节节奏、伤害飘字分离已上线，保留此前训练、战力/饰品数据同源、HUD、地图和双面试池。详见 `Docs/RELEASE-2026-09-05-DOCUMENT.md` 和 `Docs/WEBGL-RELEASE-SAFETY-2026-09-05.md`。以后用 `Tools/Stage-WebGL.mjs` 保留上一版资源及版本清单，不再手动删成四个文件。不能仅从旧 origin/master 重建覆盖官网；源码推送暂受限制，不把已发布网页等同于源码已远端同步。
...
7. 玩家可见文案使用中文，企划指定职业 `Rapper` 为明确例外。当前角色按种族与主唱/主舞/Rapper/门面显示。企划（2）的C/B/A/S招募评级、收费与刷新需在后续同一批成套对齐，不能单独恢复旧SSR/SR/R抽卡UI；旧抽卡序列化字段只作兼容用途，不是当前显示设计依据。
8. 玩家存档位于本机 `PlayerPrefs`，不会随 Git 跨设备同步。

## 3. 当前视觉选择

- 大厅：沉浸式霓虹舞台；角色是主视觉，入口使用轻量 AI 舞台装置，禁止恢复厚重大卡遮住角色。
- 选秀：线上面试、线下面试两个候选池，线上报价较低、线下较高；每日 18:00 免费更新，各池最多十名未签约候选，查看免费、签约扣除界面公示的金币。中间采用叠卡候选浏览，顶部 HUD 与底部导航沿用已确认设计，选秀导航高亮。旧三卡池、十连、概率和保底不是当前角色选秀设计。
...
- [ ] 两种面试池、候选切换、当日名单持久化、18:00 免费刷新、报价校验、签约扣费与成员入库有效；签约不能悄悄补入新人或重排未签约名单。
- [ ] 任务领取不会重复发奖，状态刷新正确。
...
- [ ] 候选姓名、种族、职业、能力与签约报价可读；左右叠卡可切换，当前候选和底部选秀导航状态一致。
